...
await page.click('nav button[data-modus="functie"]'); await page.waitForTimeout(1000)
const startTxt=await page.textContent('aside')
ok('ui: startscherm met twee opties (Excel inladen / zelf invullen per vakgroep)', /Excel inladen/.test(startTxt)&&/Zelf invullen/.test(startTxt)&&/Longfunctie/.test(startTxt)&&/Cardiologie/.test(startTxt))
ok('ui: rechts een duidelijke lege staat zolang er geen kamers zijn', /nog geen kamers/.test(await page.textContent('section')))
await page.click('aside button[data-fk-preset="Longfunctie"]'); await page.waitForTimeout(1200)
const kop=await page.textContent('aside')
ok('ui: na keuze staat stap 1 Kamers met de Longfunctie-set', /Kamers/.test(kop)&&/"Longfunctie" geladen/.test(kop))
const kamerNamen=await page.evaluate(()=>[...document.querySelectorAll('aside input[placeholder="Kamer, bv. A1.213"]')].map(i=>i.value))
ok('ui: kamers A1.243 … A1.215 staan in de invoer', kamerNamen.join()==='A1.243,A1.253,A1.213,A1.215', kamerNamen.join())
await page.click('aside button:has-text("Volgende: onderzoeken")'); await page.waitForTimeout(700)
...
await page.fill('aside input[placeholder="Zoek code of omschrijving…"]','polygrafie'); await page.waitForTimeout(400)
ok('ui: zoeken filtert de codelijst', (await page.locator('aside input[placeholder="Code"]').count())===4, 'aantal '+(await page.locator('aside input[placeholder="Code"]').count()))
await page.fill('aside input[placeholder="Zoek code of omschrijving…"]',''); await page.waitForTimeout(300)
await page.click('aside button:has-text("Zonder kamer")'); await page.waitForTimeout(400)
ok('ui: filter "zonder kamer" toont precies de codes zonder kamer', (await page.locator('aside input[placeholder="Code"]').count())===8, 'aantal '+(await page.locator('aside input[placeholder="Code"]').count()))
